...
@dag(
    ...
    2. Или поместите файл credentials в доступное место
    
    ## Процесс:
    - Извлекает данные из Google Sheets
    - Трансформирует данные
    - Загружает в ту же таблицу, что и SharePoint
    """
)
def gsheet_families_etl():
    ...
    def extract_data_from_gsheet() -> str:
        """Извлекает данные из Google Sheets используя credentials из Variables."""
        output_path = str(DATA_ROOT / "gsheet_export_families.csv")
        
        # Функция сначала попробует использовать Variable,
        # затем файл (если путь указан в Variable)
        keyfile_path = Variable.get("gsheet_service_account_path", default_var=None)
        
        return gsheet.extract_families_from_gsheet(
            sheet_key=GSHEET_KEY,
            worksheet_name=GSHEET_WORKSHEET,
            output_path=output_path,
            keyfile_path=keyfile_path  # Опциональный параметр
        )

    @task
    def transform_gsheet_data(raw_data_path: str) -> str:
        """Трансформирует данные из Google Sheets."""
