...
def extract_added_incremental(
    ...
    **context
    ...
    """
    Экспортирует новую таблицу revit.added_element с инкрементальной выгрузкой по дате
    (после 2 марта 2026).
    ...
    Returns:
        Путь к CSV файлу
    """
    return _extract_element_table_incremental(
        postgres_conn_id=postgres_conn_id,
        table="revit.added_element",
        output_path=output_path,
        last_date=last_date,
    )


def extract_modified_incremental(
    postgres_conn_id: str,
    output_path: str,
    last_date: str = None,
    **context
    ...
    """
    Экспортирует новую таблицу revit.modified_element с инкрементальной выгрузкой по дате
    (после 2 марта 2026). Структура идентична revit.added_element.
    ...
    return _extract_element_table_incremental(
        postgres_conn_id=postgres_conn_id,
        table="revit.modified_element",
        output_path=output_path,
        last_date=last_date,
    )


def _extract_element_table_incremental(
    postgres_conn_id: str,
    table: str,
    output_path: str,
    last_date: str = None,
    ...
    """
    Общая инкрементальная выгрузка для таблиц элементов (revit.added_element / revit.modified_element).
    """
    hook = PostgresHook(postgres_conn_id=postgres_conn_id)
    conn = hook.get_conn()

    if last_date:
        sql = f"SELECT * FROM {table} WHERE date > '{last_date}' ORDER BY date"
        print(f"Инкрементальная выгрузка {table}: date > '{last_date}'")
    else:
        sql = f'SELECT * FROM {table} ORDER BY date'
        print(f"Полная выгрузка {table} (первый запуск)")

    df = pd.read_sql(sql, conn)
    conn.close()

    df.to_csv(output_path, index=False, encoding='utf-8')
    print(f"Экспортировано {len(df)} записей {table} в {output_path}")

    return output_path


def extract_legacy_added_elements(
    postgres_conn_id: str,
    output_path: str,
    force_reload: bool = False,
    **context,
    ...
    """
    Экспортирует старую таблицу legacy.added_element_legacy (до 2 марта 2026).

    Идемпотентен: при повторных запусках возвращается путь к уже выгруженному CSV
    без обращения к БД (legacy не пополняется новыми записями).
    ...
    """
    if not force_reload and os.path.exists(output_path) and os.path.getsize(output_path) > 0:
        print(f"Legacy Added Elements CSV уже существует, пропускаем выгрузку: {output_path}")
        return output_path

    hook = PostgresHook(postgres_conn_id=postgres_conn_id)
    conn = hook.get_conn()
    sql = 'SELECT * FROM legacy.added_element_legacy'
    df = pd.read_sql(sql, conn)
    conn.close()

    df.to_csv(output_path, index=False, encoding='utf-8')
    print(f"Экспортировано {len(df)} СТАРЫХ записей added_element в {output_path}")
    return output_path